...
bool Window::isThirdMatrixIsValid()
{
    bool result = true;

    for (int rowIndex = 0; rowIndex < this->ui->thirdMatrix->rowCount(); ++rowIndex)
        for (int columnIndex = 0; columnIndex < this->ui->thirdMatrix->columnCount(); ++columnIndex)
        {
            if (this->ui->thirdMatrix->item(rowIndex,columnIndex) == nullptr)
            {
                QMessageBox::warning(this, tr("Ошибка"), tr("Пустое поле во второй матрице"), QMessageBox::Ok);
                return false;
            }
            ...
            {
                this->ui->thirdMatrix->item(rowIndex,columnIndex)->text().toDouble(&result);
                if (result == false)
                    QMessageBox::warning(this, tr("Ошибка"), tr("Введено некоректное значение во втророй матрице"), QMessageBox::Ok);
            }
        }
...
void Window::on_pushButton_mulMat_clicked()
{
    ...
    try {
        ...
        ui->thirdMatrix->setColumnCount(result.getW());
        ui->thirdMatrix->setRowCount(result.getH());

        ui->thirdMatrix->update();
        qApp->processEvents();

        result.install(ui->thirdMatrix);
...
void Window::on_pushButton_transpose_3_clicked()
{
    if (!isSecondMatrixValid()) return;
    if (this->ui->secondMatrix->rowCount() != this->ui->secondMatrix->columnCount())
    {
        QMessageBox::warning(this, tr("Ошибка"), tr("Введённая матрица не квадратная"), QMessageBox::Ok);
        return;
    }

     DMatrix m(ui->secondMatrix);
     m = m.transpose();
     m.install(ui->secondMatrix);
}

void Window::on_pushButton_det_3_clicked()
{
    if (!isSecondMatrixValid()) return;
    if (this->ui->secondMatrix->rowCount() != this->ui->secondMatrix->columnCount())
    {
...
void Window::on_pushButton_inver_3_clicked()
{
    if (!isSecondMatrixValid()) return;
    DMatrix m(ui->secondMatrix);
    try {
...
void Window::on_pushButton_scalarmul_5_clicked()
{
    if (!isSecondMatrixValid()) return;

    bool isValidScalar = false;
    double scalarValue = this->ui->scalar_value_5->text().toDouble(&isValidScalar);
    if (!isValidScalar)
    {
        QMessageBox::warning(this, tr("Ошибка"), tr("Введено некоректное значение"), QMessageBox::Ok);
        return;
    ...
    for (int rowIndex = 0; rowIndex < this->ui->secondMatrix->rowCount(); ++rowIndex)
        for (int columnIndex = 0; columnIndex < this->ui->secondMatrix->columnCount(); ++columnIndex)
        {
            double previousValueInCell = this->ui->secondMatrix->item(rowIndex,columnIndex)->text().toDouble();
            double newValue = previousValueInCell * scalarValue;
            this->ui->secondMatrix->item(rowIndex,columnIndex)->setText(QString::number(newValue));
        }
}
...
void Window::on_pushButton_clicked()
{
    ...
    ui->firstMatrix->setRowCount(ui->rows_spinBox->value());
    ui->firstMatrix->setColumnCount(ui->column_spinbox->value());
    ui->thirdMatrix->setRowCount(ui->rows_spinBox_4->value());
    ui->thirdMatrix->setColumnCount(ui->column_spinbox_4->value());
